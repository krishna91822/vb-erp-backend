...
let reviews = require("../models/ReviewModel");
...
describe("/PATCH/:id reviews", () => {
  it("it should UPDATE the status of the given object ID", (done) => {
    let Idreview = new reviews({
      reqName: "admin",
      reqType: "profile-creation",
      status: "pending",
      reqId: 15,
    ...
    Idreview.save((err, Idreview) => {
      chai
        .request(app)
        .put("/reviews/" + Idreview.id)
        .send({
          reqName: "admin",
          reqType: "profile-creation",
          status: "accepted",
